...
## Updated for Regex Sentiment Analysis - May 2026
##
## Reference: 
...
class RDRNode:
    ...
    def evaluate(self, row):
        ...
        try:
            ...
            if op == '>':  return float(row_val) > float(val)
            # Replaced 'contains' with 'regex' for more flexible sentiment matching
            if op == 'regex': 
                return bool(re.search(str(val), str(row_val), re.IGNORECASE))
        except:
...
class SCRDR_Engine:
    ...
    def classify(self, row):
        ...
        while curr:
            if curr.evaluate(row):
                last_match = curr
                curr = curr.if_true # Try refinements (Exceptions)
            else:
                curr = curr.if_false # Try alternatives (New paths)
        return last_match
    ...
    def add_rule(self, row, last_node):
        ...
        # 2. Operator - 'contains' removed, 'regex' added
        op = get_validated_input("Enter operator (==, <, >, regex): ", options=['==', '<', '>', 'regex'])
        if op == 'EXIT_COMMAND': return False
        
        # 3. Value (Regex Pattern or Threshold)
        if op == 'regex':
            val = get_validated_input("Enter Regex pattern (e.g., (ဝမ်းနည်း|စိတ်ညစ်)): ")
        else:
        ...
        if last_node.evaluate(row):
            # Predicted node matched (but was wrong) -> This is a REFINEMENT (if_true)
            ...
            # Predicted node did not match -> This is a new ALTERNATIVE (if_false)
            curr = last_node
            while curr.if_false: curr = curr.if_false
            curr.if_false = new_node
        return True
...
3. Use --mode test to evaluate a built model without being prompted for rules.
    ...
    parser = argparse.ArgumentParser(
        description="Interactive RDR Knowledge Builder (Regex Enabled)",
