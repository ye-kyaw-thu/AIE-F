...
─────────
  ...
  2. Multi-experiment accuracy comparison bar chart
...
─────
  ...
  # Compare multiple experiments (auto-formats names)
  python src/plot_results.py \
      --exp results/exp_bilstm results/exp_transformer results/exp_stgcn \
      --output results/comparison.png

  # Compare with fully custom names
  python src/plot_results.py \
      --exp results/exp_bilstm results/exp_transformer results/exp_stgcn \
      --names "My BiLSTM" "My Transformer" "My ST-GCN" \
      --output results/comparison.png
"""
...
def format_model_name(name: str) -> str:
    ...
    # Handle specific acronyms to keep them nicely formatted
    acronyms = {
        'bilstm': 'BiLSTM',
        'stgcn': 'ST-GCN',
    ...
    # For compound names like "stgcn_v2" or "bilstm_dropout"
...
def normalize_accuracy(value, default=0):
    ...
    if value is None:
        return default
    # If value is already in percentage range (>1), keep it
    if value > 1.0:
        return value
...
def plot_training_curves(exp_dir: Path, save_path: Optional[str] = None):
    ...
    # Filter out None values for epochs
    ...
    # Check if we have any valid train_loss data
    ...
    if has_val_acc:
        ...
        # Find best epoch only if we have val_top1 data
...
#    """Bar chart comparing best val-top1 across experiments."""
#    names       = []
#    best_top1s  = []
#    cv_means    = []
#    cv_stds     = []
...
#        elif history:
            # Try multiple possible key names for val_top1
#            val_top1s = []
#            for h in history:
...
#                cv_stds.append(None)
#            else:
#                print(f"No valid accuracy data in {exp_dir}, skipping")
#        else:
#            print(f"No data in {exp_dir}, skipping")

# Adjust with compare_result.py code.
# compare_results.py reads metrics_val.json
# plot_results.py reads history.json
# history.json is created by train.py during training, which calculates Avg of Batch Accuracies.
# *Mathematically exact.
#
# Example: Why they differ?
# Imagine 3 batches with sizes [32, 32, 20]
# Batch 1: 30/32 = 93.75% Batch accuracy
# Batch 2: 31/32 = 96.88% 
# Batch 3: 20/20 = 100.00%
#
# Training log (history.json)
# (03.75 + 96.88 + 100.0)/3 = 96.88%
# 
# Evaluation (metrics_val.json):
# (30 + 31 + 20)/(32 + 32 + 20) = 81/84 = 96.43%
# Difference: 0.45% 
#
# The number in the table (metrics_val.json) are more accurate!
# This is because, using sklearn's exact global calculation.
# They are computed in a single dedicated evaluation pass.
# This is the standard way to report metrics in papers.
...
def plot_comparison(exp_dirs: List[Path], output: str, custom_names: Optional[List[str]] = None):
    ...
    for i, exp_dir in enumerate(exp_dirs):
        # Try metrics_val.json FIRST (more accurate)
        metrics_path = exp_dir / 'evaluation' / 'metrics_val.json'
        cv           = load_cv_summary(exp_dir)
        ...
        if metrics_path.exists():
            # Use the precise evaluation metrics
            ...
            # Fallback to history.json (less accurate)
            history = load_history(exp_dir)
            if history:
                val_top1s = []
                for h in history:
    ...
    # Removed label='Best Val Top-1 (%)' here as it's redundant
    ...
    ax.set_title('Model Comparison — Val Top-1 Accuracy', fontsize=14, fontweight='bold')
    ax.set_ylim(0, min(105, max(best_top1s) + 10))
    ax.grid(axis='y', alpha=0.3)
    
    # Only show legend if CV error bars were actually plotted
    if cv_label_added:
...
def plot_cv_distribution(exp_dir: Path):
    ...
    # Try multiple possible key names for fold scores
...
def main():
    ...
    if len(exp_dirs) > 1:
        ...
        if valid:
            plot_comparison(valid, args.output, args.names)
